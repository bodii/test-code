...
//// <reference path="..." /> // 用于声明文件间的 依赖。

/*
    三斜线引用告诉编译器在编译过程中要引入的额外的文件。
    当使用 --out或 --outFile时，它也可以做为调整输出内容顺序的一种方法。文件在输出文件内容中的位置与经过预
    处理后的输入
    顺序一致。
    ...
    这个过程会以一些根文件开始；它们是在命令行中指定的文件或是在tsconfig.json中的“file”列表里的文件。这
    些根文件按指定的顺序进行预处理。在一个文件被加入列表前，它包含的所有三斜线引用都要被处理，还有它们包含的目标。
    三斜线引用以它们在文件里出现的顺序，使用深度优先的方式解析。

    一个三斜线引用路径是相对包含它的文件的，如果不是根文件。


    【 错误 】
    引用不存的文件会报错。一个文件用三斜线指令引用自己会报错。
    ...
    当传递了 --skipDefaultLibCheck时，编译器会忽略检查带有 /// 《reference no-default-lib="true" />的文件。

    【 /// <amd-module /> 】
    默认情况下生成的AMD模块都是匿名的。但是，当一些工具需要处理生成的模块时会产生问题。如r.js.
