...
use async_graphql::{Context, Enum, Error, Object, Result, Schema, Subscription, ID};
...
impl MutationRoot {
    ...
    async fn delete_book(&self, ctx: &Context<'_>, id: ID) -> Result<bool, Error> {
        let mut books = ctx.data_unchecked::<Storage>().lock().await;
...
impl BookChanged {
    ...
    async fn book(&self, ctx: &Context<'_>) -> Result<Option<Book>, Error> {
        let books = ctx.data_unchecked::<Storage>().lock().await;
