...
var reconnectServer = function(self, state) {
  ...
  state.pool.once('connect', function() {
    ...
    var authenticate = function(retries, interval, provider, connections, self, state, callback) {
      retries = retries - 1;
      if(retries == 0) return callback(new Error('failed to authenticate'));

      // Attemp to reauthenticate
    ...
    for(var i = 0; i < keys.length; i++) {
      authenticate(state.authenticationRetries, state.authenticationRetryIntervalMS
        , state.authProviders[keys[i]], connections, self, state, function(err, i) {
          count = count - 1;
