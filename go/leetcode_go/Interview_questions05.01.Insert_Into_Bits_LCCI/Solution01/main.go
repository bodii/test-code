...
func insertBits(N int, M int, i int, j int, successResult int) int {
	...
	MBinary := strconv.FormatInt(int64(M), 2)
	last := len(NBinary) - 1
	fmt.Println("last:", last)
	fmt.Println("nBinary:", NBinary, " MBinary:", MBinary)
	fmt.Println("successResult:", strconv.FormatInt(int64(successResult), 2), successResult)
	strRes := ""
	if last-j > 0 {
		strRes = NBinary[:last-j]
	}
	fmt.Println("01: ", strRes)
	for n := 0; n <= j-i-len(MBinary); n++ {
		strRes += "0"
	}
	strRes += MBinary
	fmt.Println("last-i+1:", last-i+1)
	if last-i+1 >= 0 {
		fmt.Println("01: ", strRes, " 02: ", MBinary, " 03: ", NBinary[last-i+1:])
		strRes += NBinary[last-i+1:]
	} else {
		fmt.Println("01: ", strRes, " 02: ", MBinary, " 03: ", NBinary)
		for n := last - i + 1; n < 0; n++ {
			strRes += "0"
		}
		strRes += NBinary
		fmt.Println("01: ", strRes, " 02: ", MBinary, " 03: ", NBinary)
	...
	result, _ := strconv.ParseInt(strRes, 2, 32)
	fmt.Println("result binary:", strRes)
