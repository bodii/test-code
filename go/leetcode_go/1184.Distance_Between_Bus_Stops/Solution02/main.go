...
func distanceBetweenBusStops(distance []int, start int, destination int) int {
	left, right := 0, 0
	len := len(distance)
	leftIndex, rightIndex := start%len, destination%len

	for i := 0; leftIndex != destination; {
		fmt.Println("left:", distance[leftIndex])
		left += distance[leftIndex]
		i++
		leftIndex = (start + i) % len
	}

	for i := 0; rightIndex != start; {
		fmt.Println("right:", distance[rightIndex])
		right += distance[rightIndex]
		i++
		rightIndex = (destination + i) % len
	}

	// fmt.Println("right  val:", right, "left val:", left)
