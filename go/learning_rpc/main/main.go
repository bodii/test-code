...
func broadcast2(addr1, addr2 string) {
...
func startServer03(registryAddr string, wg *sync.WaitGroup) {
	var foo Foo
	l, _ := net.Listen("tcp", ":0")
	server := learning_rpc.NewServer()
	_ = server.Register(&foo)
...
func call3(registry string) {
	d := xclient.NewLearningRegisteryDiscovery(registry, 0)
	xc := xclient.NewXClient(d, xclient.RandomSelect, nil)
	defer func() { _ = xc.Close() }()
...
func broadcast3(registry string) {
	d := xclient.NewLearningRegisteryDiscovery(registry, 0)
	xc := xclient.NewXClient(d, xclient.RandomSelect, nil)
	defer func() { _ = xc.Close() }()
...
func test2() {
	...
	time.Sleep(time.Second)
	call2(addr1, addr2)
	broadcast2(addr1, addr2)
}

func test3() {
	log.SetFlags(0)
	registryAddr := "http://localhost:9999/_learning_rpc_/registry"
	var wg sync.WaitGroup
	wg.Add(1)
	go startRegistry(&wg)
	wg.Wait()
	...
	time.Sleep(time.Second)
	wg.Add(2)
	go startServer03(registryAddr, &wg)
	go startServer03(registryAddr, &wg)
	wg.Wait()

	time.Sleep(time.Second)
	call3(registryAddr)
	broadcast3(registryAddr)
}
