...
Title;             创建一个简单的 REST 接口
Issue:你想使用一个简单的 REST 接口通过网络远程控制或访问你的应用程序，但是你
又不想自己去安装一个完整的 web 框架
Answer: 构建一个 REST 风格的接口最简单的方法是创建一个基于 WSGI 标准（PEP
3333）的很小的库，下面是一个例子
"""
...
if __name__ == '__main__':
    ...
    dispathcher = PathDispatcher()
    dispathcher.register('GET', '/hello', hello_world)
    dispathcher.register('GET', '/localtime', localtime)

    httpd = make_server('', 8080, dispathcher)
...
级简单了，只要你遵循 start response() 函数的编写规则，并且最后返回字节字符串
即可。
